...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        sizeB = len(state.buffer)
        sizeS = len(state.stack)
        if sizeB >= 3:
            buf = []
            indcs = state.buffer[::-1][0:3]
            for i in indcs:
                if words[i] not in self.word_vocab.keys() and words[i] != None:
                    buf.append(self.pos_vocab[pos[i]])
                elif words[i] == None:
                    buf.append(3)
                else:
                    buf.append(self.word_vocab[words[i]])
        else:
            buf = []
            indcs = state.buffer[::-1][0:sizeB]
            for i in indcs:
                if words[i] not in self.word_vocab.keys() and words[i] != None:
                    buf.append(self.pos_vocab[pos[i]])
                elif words[i] == None:
        ...
        if sizeS >= 3:
            ...
            for i in indcs:
                if words[i] not in self.word_vocab.keys() and words[i] != None:
                    stac.append(self.pos_vocab[pos[i]])
                elif words[i] == None:
                    stac.append(3)
                else:
                    stac.append(self.word_vocab[words[i]])
        else:
            stac = []
            indcs = state.stack[::-1][0:sizeS]
            for i in indcs:
                if words[i] not in self.word_vocab.keys() and words[i] != None:
                    stac.append(self.pos_vocab[pos[i]])
                elif words[i] == None:
                    ...
                    stac.append(self.word_vocab[words[i]])
            for i in range(3-sizeS):
                stac.append(4)     
               
        ret = np.array(stac+buf)
    ...
    def get_output_representation(self, output_pair):  
        ...
        if  output_pair[0] == "shift":
            ...
            ret[dep_relations.index(output_pair[1])] = 1
        else:
            ret[dep_relations.index(output_pair[1])+45] = 1
        return ret
